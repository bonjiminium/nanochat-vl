...
def ensure_base():
    ...
    subprocess.run(["python", "-m", "scripts.base_train", "--depth=2", "--aspect_ratio=64", "--head_dim=64", "--max_seq_len=64", "--vocab_size=4096", "--device_batch_size=4", "--total_batch_size=256", "--num_iterations=20", "--warmup_ratio=0.1", "--warmdown_ratio=0.2", "--eval_every=-1", "--core_metric_every=-1", "--save_every=20"], check=True)

def ensure_mid():
    ensure_base()
    if has_checkpoint("mid"): return
    subprocess.run(["python", "-m", "scripts.mid_train", "--num_iterations=10", "--device_batch_size=4", "--total_batch_size=256", "--max_seq_len=64", "--eval_every=-1", "--save_every=10"], check=True)

def ensure_sft():
    ensure_mid()
    if has_checkpoint("sft"): return
    subprocess.run(["python", "-m", "scripts.chat_sft", "--num_iterations=10", "--device_batch_size=4", "--total_batch_size=256", "--max_seq_len=64", "--eval_every=-1", "--eval_steps=4", "--save_every=10"], check=True)
...
def test_base():
    setup_env()
    ensure_tokenizer()
    subprocess.run(["python", "-m", "scripts.base_train", "--depth=2", "--aspect_ratio=64", "--head_dim=64", "--max_seq_len=64", "--vocab_size=4096", "--device_batch_size=4", "--total_batch_size=256", "--num_iterations=20", "--warmup_ratio=0.1", "--warmdown_ratio=0.2", "--eval_every=5", "--eval_tokens=256", "--core_metric_every=-1", "--save_every=20", "--resume_from=-2"], check=True)
    volume.commit()
...
def test_mid():
    setup_env()
    ensure_base()
    subprocess.run(["python", "-m", "scripts.mid_train", "--num_iterations=10", "--device_batch_size=4", "--total_batch_size=256", "--max_seq_len=64", "--eval_every=5", "--eval_tokens=256", "--save_every=10"], check=True)
    volume.commit()
...
def test_sft():
    setup_env()
    ensure_mid()
    subprocess.run(["python", "-m", "scripts.chat_sft", "--num_iterations=10", "--device_batch_size=4", "--total_batch_size=256", "--max_seq_len=64", "--eval_every=5", "--eval_steps=4", "--save_every=10"], check=True)
    volume.commit()
...
def test_vl():
    setup_env()
    ensure_sft()
    subprocess.run(["python", "-m", "scripts.vl_train", "--num_steps=20", "--batch_size=2", "--grad_accum=2", "--max_seq_len=64", "--use_muon=0", "--print_every=5"], check=True)
    volume.commit()
