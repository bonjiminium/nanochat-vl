...
all_text = [("news", news_text), ("korean", korean_text), ("code", code_text), ("math", math_text), ("science", science_text), ("fwe-train", train_text)]
if val_text: all_text.append(("fwe-val", val_text))

tokenizer_results, vocab_sizes = {}, {}
GREEN, RED, RESET = '\033[92m', '\033[91m', '\033[0m'

for tokenizer_name in ["gpt2", "gpt4", "ours"]:
    if tokenizer_name == "gpt2": tokenizer = RustBPETokenizer.from_pretrained("gpt2")
    elif tokenizer_name == "gpt4": tokenizer = RustBPETokenizer.from_pretrained("cl100k_base")
    else: tokenizer = get_tokenizer()
    ...
    for name, text in all_text:
        ...
        tokenizer_results[tokenizer_name][name] = dict(bytes=nbytes, tokens=len(encoded), ratio=nbytes / len(encoded))
...
def print_comparison(baseline_name, baseline_results, ours_results):
    ...
    print("-" * 95)
    for name, text in all_text:
        b, o = baseline_results[name], ours_results[name]
        relative_diff = ((b['tokens'] - o['tokens']) / b['tokens']) * 100
        if b['ratio'] > o['ratio']: baseline_color, ours_color, better, diff_color = GREEN, RED, baseline_name, RED
        elif o['ratio'] > b['ratio']: baseline_color, ours_color, better, diff_color = RED, GREEN, "Ours", GREEN
        else: baseline_color, ours_color, better, diff_color = "", "", "Tie", ""
        print(f"{name:<10} {b['bytes']:<8} {baseline_color}{b['tokens']:<7}{RESET} {baseline_color}{b['ratio']:<7.2f}{RESET} {ours_color}{o['tokens']:<7}{RESET} {ours_color}{o['ratio']:<7.2f}{RESET} {diff_color}{relative_diff:+7.1f}%{RESET}     {better:<10}")

print_comparison("GPT-2", tokenizer_results['gpt2'], tokenizer_results['ours'])
print_comparison("GPT-4", tokenizer_results['gpt4'], tokenizer_results['ours'])

lines = []
for baseline_name in ["GPT-2", "GPT-4"]:
    baseline_key = baseline_name.lower().replace('-', '')
    baseline_results, ours_results = tokenizer_results[baseline_key], tokenizer_results['ours']
    lines.append(f"### Comparison with {baseline_name}")
    lines.append("")
    lines.append(f"| Text Type | Bytes | {baseline_name} Tokens | {baseline_name} Ratio | Ours Tokens | Ours Ratio | Relative Diff % |")
    lines.append("|-----------|-------|--------------|--------------|-------------|------------|-----------------|")
    for name, text in all_text:
        b, o = baseline_results[name], ours_results[name]
        relative_diff = ((b['tokens'] - o['tokens']) / b['tokens']) * 100
        lines.append(f"| {name} | {b['bytes']} | {b['tokens']} | {b['ratio']:.2f} | {o['tokens']} | {o['ratio']:.2f} | {relative_diff:+.1f}% |")
    lines.append("")
